...
finetune = False
pretrained_weight = "/Data3/cao/ZiHanCao/huawei_contest/code3/zihan/ckpts/TransformerP1/ep_1_iter3299.pth"
if finetune:
    assert pretrained_weight != "", "pretrained_weight should not be empty"
prefix_weight_name = "TransformerAll"
if finetune:
...
# angle_dim = 128
# range_dim = 150
factor = 20 if not finetune else 100
slice_num = 1350
slice_num_r1 = 0
...
MEAN = np.array([-5.091097, -5.1093035, -5.0822234, -5.0828133], dtype=np.float32)[
    np.newaxis, np.newaxis, np.newaxis, np.newaxis
]
STD = np.array([5.861487, 5.879711, 5.8663826, 5.875907], dtype=np.float32)[
    np.newaxis, np.newaxis, np.newaxis, np.newaxis
]
# to tensor
...
class TrainDataset(Dataset):
    def __init__(self):
        data_dir1 = r"/Data3/cao/ZiHanCao/huawei_contest/data/Round3Pos1/train.h5"
        file1 = h5py.File(data_dir1, "r")['data'][:]
        data_dir2 = r"/Data3/cao/ZiHanCao/huawei_contest/data/Round3Pos2/train.h5"
        file2 = h5py.File(data_dir2, "r")['data'][:]
        data_dir3 = r"/Data3/cao/ZiHanCao/huawei_contest/data/Round3Pos3/train.h5"
        ...
        global_idx_to_local = {}
        total_len = file1.shape[0] +file2.shape[0]+file3.shape[0]+file4.shape[0] +file5.shape[0] +file6.shape[0]
        for i in range(total_len):
            ...
            elif i < file1.shape[0] +file2.shape[0] +file3.shape[0]+file4.shape[0]:
                global_idx_to_local[i] = (3, i - file1.shape[0]-file2.shape[0]-file3.shape[0])
            elif i < file1.shape[0]+file2.shape[0] +file3.shape[0]+file4.shape[0] + file5.shape[0]:
                global_idx_to_local[i] = (4, i - file1.shape[0]-file2.shape[0]-file3.shape[0]-file4.shape[0])
            else:
                global_idx_to_local[i] = (5, i - file1.shape[0]-file2.shape[0]-file3.shape[0]-file4.shape[0]-file5.shape[0])
        self.global_idx_to_local = global_idx_to_local
                
        data_files = [file1,file2,file3,file4,file5,file6]
        self.data_files = data_files
        
        truth_lines = read_slice_of_file(anchor_path,0, 1350)
        truth_lines2 = read_slice_of_file(anchor_path2, 0, 2760)
        self.absolut_pos = np.concatenate((np.loadtxt(truth_lines).reshape(slice_num, 2),np.loadtxt(truth_lines2).reshape(slice_num_r2, 2)),axis=0)
            
        # global MEAN, STD, GT_MEAN, GT_STD
        # MEAN = torch.tensor(np.mean(self.datalist, axis=(0, 1, 2, 3), keepdims=True))
        # STD = torch.tensor(np.std(self.datalist, axis=(0, 1, 2, 3), keepdims=True))
        # GT_MEAN = torch.tensor(np.mean(self.absolut_pos, axis=0, keepdims=True))
        # GT_STD = torch.tensor(np.std(self.absolut_pos, axis=0, keepdims=True))
...
class TestDataset(Dataset):
    ...
    def __init__(self):
        data_dir1 = r"/Data3/cao/ZiHanCao/huawei_contest/data/Round3Pos1/test.h5"
        file1 = h5py.File(data_dir1, "r")['data'][:]
        ...
        self.datalist = []
        truth_lines = read_slice_of_file(anchor_path,0,150)
        ...
        if not opt.eval_only:
            ...
            for batch_idx, (data, target) in enumerate(train_loader, 1):
                ...
                with torch.autocast(
                    ...
                    # loss functions
                    loss = critrtion(output1[:, 0], target[:, 0]) + critrtion(
                        output1[:, 1], target[:, 1]
                    )
                ...
                if (batch_idx + 1) % 100 == 0:
                    ...
                    with torch.no_grad():
                        ...
                        for t_batch_idx, (data, target) in enumerate(test_loader, 1):
                            ...
                            target = (
                                target.cuda(device=opt.device).float().reshape(1, 2)
                            )
                    ...
                    # ema_params = set_ema_model_params_with_keys(ema_model.state_dict(), keys=saved_keys)
