...
class SegFormer3D(nn.Module):
    def __init__(
        ...
        self.proj = nn.Sequential(
            nn.Conv3d(sum(reversed_embed_dims), embed_dims[-1], kernel_size=1),
    ...
    def forward(self, x):
        ...
        c2 = nn.functional.interpolate(c2, size=max_size, mode='trilinear', align_corners=False)
        c3 = nn.functional.interpolate(c3, size=max_size, mode='trilinear', align_corners=False)
        c4 = nn.functional.interpolate(c4, size=max_size, mode='trilinear', align_corners=False)
